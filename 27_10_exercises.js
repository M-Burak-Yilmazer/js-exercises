...
input1.forEach((item) => console.log(item, item ** 2));
let result = input1.map((item) => item ** 2);
console.log(result);
...
console.log(
  input2.filter((item) => item > 0).reduce((sum, int) => sum + int, 0)
);
const sonuc = input2.reduce((sum, int) => int > 0 && sum + int);
console.log(sonuc);
//result 42;

//
//?Calculate the mean=average and median=ortadaki sayı values of the number elements from the input array.
const input3 = [12, 46, 32, 64];
input3.sort();
console.log(input3);

const resul = input3.reduce(
  (sum, int, index, array) => {
...
// console.log(convertArrayToObject(students1))[
//   ({
//     name: "David",
